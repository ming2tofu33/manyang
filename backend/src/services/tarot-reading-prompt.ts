...
function getStyleContract(spread: TarotReadingSpread): string[] {
  ...
  if (spread === "question_one_card") {
    return [
      ...
      "overview는 4문장 안팎으로 구성하세요. 첫 문장은 선택한 질문에 바로 답하세요. 이어서 카드와 정방향/역방향의 상징 근거, 오늘 실제로 알아차릴 수 있는 신호, 오늘 확인할 기준 순서로 이어 쓰세요.",
      "전체 예시 리딩을 따라 쓰지 말고 표현 단위만 참고하세요. 어색한 예: 불씨의 흥분, 가능성의 흥분이 올라오면, 그 불씨가 실제로 살아있느냐는 점. 자연스러운 예: 막 붙기 시작한 의욕, 마음이 자꾸 그쪽으로 기울면, 그 생각이 오늘도 힘을 갖고 있는지.",
      "시각적으로는, ~라는 점입니다, ~을 중심으로 합니다 같은 번역투 설명형 표현은 피하세요.",

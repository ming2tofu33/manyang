...
- 런타임 타로 리딩은 `tarotCards` 통합 덱(메이저+마이너 78장)을 기준으로 동작한다.
- 마이너 카드 이미지는 `frontend/public/manyang/tarot/minor/` 아래에 있고, 배경 제거 산출물과 asset mapping이 연결되어 있다.
- 오늘의 한 장과 질문별 한 장은 78장 전체 덱에서 카드를 뽑는다.
...
- [x] `TarotMajorCard` 중심 타입을 `TarotCard` 공통 타입으로 확장한다.
- [x] `arcana: "major" | "minor"`를 추가한다.
- [x] 마이너 카드는 `suit: "wands" | "cups" | "swords" | "pentacles"`를 가진다.
- [ ] `deckMode: "major_only" | "minor_only" | "full_78"` 개념을 추가한다.
- [x] 기존 저장된 메이저 카드 reading이 계속 열리도록 legacy id 호환 레이어를 둔다.
- [x] 테스트를 추가한다: 78장 개수, 중복 `cardKey` 없음, 이미지 경로 존재, legacy major id 로드 가능.
...
- [x] 오늘의 한 장 draw 로직이 `tarotMajorCards`가 아니라 `full_78` 덱을 사용할 수 있게 바꾼다.
- [x] 저장 payload에 `cardKey`를 포함한다.
- [x] 기존 저장 데이터에 숫자 id만 있어도 결과 화면에서 정상 표시되게 한다.
- [x] 결과 문구는 마이너 카드가 나와도 가볍고 일상적인 톤으로 읽히게 조정한다.
- [x] 78장 확장 후 체감 품질을 확인한다: 메이저는 큰 흐름, 마이너는 작은 상황/감정/현실 조언으로 느껴져야 한다.

### Phase 4. 질문별 한 장 MVP

- [x] 첫 화면은 질문을 바로 나열하지 않고 상태를 먼저 고르게 한다.
- [x] 상태 카드는 6개로 시작한다: 마음이 복잡해, 관계가 신경 쓰여, 일이 막힌 느낌이야, 돈이나 현실이 불안해, 선택을 앞두고 있어, 그냥 오늘의 신호가 궁금해.
- [x] 상태를 고르면 질문 5개를 보여준다.
- [x] 질문 문안은 [[Tarot-Question-Prompts]]를 기준으로 구현한다.
- [x] 결과 화면에는 선택한 질문, 뽑은 카드, 짧은 해석, 오늘 볼 포인트를 보여준다.
- [x] 질문별 한 장도 저장/공유 진입점을 가진다.

### Phase 5. 카드 기록과 회고

- [x] 저장 데이터에 리딩 종류를 구분한다: `daily_one_card`, `question_one_card`, `daily_three_card`.
- [x] 질문별 한 장은 선택 상태, 질문, 카드, 작성 시간을 저장한다.
- [ ] 사용자가 한 줄 회고를 남길 수 있게 한다.
- [ ] 기록 화면에서 오늘의 카드와 질문별 카드가 섞여 보여도 구분 가능해야 한다.
- [x] 같은 질문을 반복해서 뽑는 UX는 과몰입을 만들 수 있으므로 제한 또는 부드러운 안내를 둔다.

### Phase 6. 공유용 콘텐츠

- [x] 공유 결과는 짧게 만든다: 질문, 카드명, 한 문장 메시지.
- [x] 공유 이미지는 카드 이미지가 중심이고, 앱 설명 문구는 최소화한다.
- [x] 공유 문구는 결과 전체를 복사하는 방식보다 다시 들어오고 싶게 만드는 짧은 티저를 우선한다.
- [ ] 공유 이벤트를 기록한다: `tarot_share_clicked`, `tarot_share_completed`.

## API와 저장 계약 체크리스트

- [x] API validator가 메이저 카드 id만 허용하는 구조인지 확인한다.
- [x] `cardKey` 기반 저장을 추가하되 기존 숫자 id를 깨지 않게 한다.
- [x] spread 타입에 `question_one_card`를 추가한다.
- [x] 질문별 리딩에는 `questionKey`, `questionText`, `stateKey`를 저장한다.
- [x] 서버에 저장하는 구조와 localStorage 구조가 달라지지 않도록 공통 타입을 둔다.

구현 메모:

- 질문별 한 장 리딩 spread는 `question_one_card`를 사용한다.
- 질문별 무료 사용량 feature key는 `tarot_question_one_card`를 사용한다.
- 광고 기반 추가 뽑기를 위해 `unlockMethod: "rewarded_ad"` 값을 타입에 포함했지만, 현재 MVP에서는 서버가 비관리자 요청을 `daily_free`로 정규화하고 이미 사용한 경우 `rewarded_ad_required`를 반환한다.

## UI 체크리스트

- [x] 오늘의 한 장과 질문별 한 장은 서로 다른 진입점으로 둔다.
- [x] 질문별 한 장 첫 화면은 카테고리명이 아니라 사용자 상태 문장으로 보여준다.
- [x] 질문 선택 화면은 5개를 넘기지 않는다.
- [x] 결과 화면은 설명이 길어지지 않게 카드 메시지, 오늘 볼 포인트, 저장/공유 CTA만 우선한다.
- [x] 3장 리딩 CTA는 결과 하단에서 자연스럽게 연결하되 무료 결과를 빈약하게 만들지 않는다.
...
- [x] 전체 78장 카드가 런타임 데이터에 존재한다.
- [x] 오늘의 한 장이 78장 전체 덱에서 동작한다.
- [x] 기존 메이저 카드 저장 결과가 깨지지 않는다.
- [x] 질문별 한 장 리딩이 상태 선택, 질문 선택, 카드 결과, 저장까지 이어진다.
- [x] 3장 리딩은 유료/심화 위치를 유지한다.

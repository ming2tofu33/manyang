export const DREAM_LOADING_CAT_SCENE_MS = 5000;
export const DREAM_LOADING_ORB_MINIMUM_MS = 10000;
export const DREAM_LOADING_MINIMUM_MS = DREAM_LOADING_CAT_SCENE_MS + DREAM_LOADING_ORB_MINIMUM_MS;
...
const CAT_SCENE_MESSAGE = "고양이가 첫 장면을 살펴보고 있어요.";
...
export type DreamLoadingScene = "cat" | "orb";
...
export function getDreamLoadingSequence(elapsedMs: number): DreamLoadingSequence {
  ...
  const isCatScene = safeElapsedMs < DREAM_LOADING_CAT_SCENE_MS;
  const orbElapsedMs = Math.max(0, safeElapsedMs - DREAM_LOADING_CAT_SCENE_MS);
  const stepIndex = isCatScene
    ? 0
    : Math.min(dreamLoadingOrbSteps.length - 1, Math.floor(orbElapsedMs / ORB_STEP_MS));

  return {
    scene: isCatScene ? "cat" : "orb",
    stepIndex,
    stepLabel: `${stepIndex + 1}/${dreamLoadingOrbSteps.length}`,
    message: isCatScene ? CAT_SCENE_MESSAGE : dreamLoadingOrbSteps[stepIndex],
